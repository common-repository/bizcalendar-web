...
class Address
{
    ...
    public function createXmlElement (\DOMDocument $xmlDoc, $nodeName)
    {
        ...
        if ($this->type == null)
        {
            throw new \Exception('Invalid address type', (int)self::ERROR_INVALID_ADDRESS_TYPE);
        } elseif ($this->type != self::TYPE_COMPANY && $this->type != self::TYPE_PERSON)
        {
            throw new \Exception('Invalid address type', (int)self::ERROR_INVALID_ADDRESS_TYPE_VALUE);
        }

...
class Sms extends PaymentAbstract  
{
	...
	protected function _loadFromXml(\DOMElement $elem)
	{
		...
		if($elems->length != 1)
		{
			throw new \Exception('Sms::loadFromXml failed: service is missing', (int)self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
		}
	...
	protected function _loadFromQueryString($queryString)
	{
        ...
        {
        	throw new \Exception('Sms::loadFromQueryString failed: signature is missing', (int)self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
        }
        $this->signature = $reqParams['signature'];
        if(!isset($reqParams['service']))
        {
        	throw new \Exception('Sms::loadFromQueryString failed: service is missing', (int)self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
        }
        $this->service = $reqParams['service'];
        if(!isset($reqParams['tran_id']))
        {
        	throw new \Exception('Sms::loadFromQueryString failed: empty order id', (int)self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
        }
	...
	protected function _prepare()
	{
		...
		{
			throw new \Exception('One or more mandatory properties are invalid!', (int)self::ERROR_PREPARE_MANDATORY_PROPERTIES_UNSET);
		}

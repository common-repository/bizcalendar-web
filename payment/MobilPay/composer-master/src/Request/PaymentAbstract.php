...
abstract class PaymentAbstract
{
	...
	public function __construct()
	{
		...
		// srand($seed);
        $this->_requestIdentifier = md5(uniqid(wp_rand(), true));
	...
	static public function factoryFromEncrypted($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null, $cipher_algo = 'rc4', $iv = null)
	{
		...
		if($privateKey === false)
        {
        	throw new \Exception('Error loading private key', (int)self::ERROR_CONFIRM_LOAD_PRIVATE_KEY);
        }
		...
		if($srcData === false)
		{
			...
			throw new \Exception('Failed decoding data', (int)self::ERROR_CONFIRM_FAILED_DECODING_DATA);
		}
        
		$srcEnvKey = base64_decode($envKey);
		if($srcEnvKey === false)
		{
			throw new \Exception('Failed decoding envelope key', (int)self::ERROR_CONFIRM_FAILED_DECODING_ENVELOPE_KEY);
		}

		$srcIv = base64_decode($iv ?? '');
		if($srcIv === false)
		{
			throw new \Exception('Failed decoding initialization vector', (int)self::ERROR_CONFIRM_FAILED_DECODING_IV);
		}
		...
		if($result === false)
		{
			throw new \Exception('Failed decrypting data', (int)self::ERROR_CONFIRM_FAILED_DECRYPT_DATA);
		}
	...
	static protected function _factoryFromXml(\DOMDocument $xmlDoc)
	{
		...
		if($elems->length != 1)
		{
			throw new \Exception('factoryFromXml order element not found', (int)PaymentAbstract::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND);
		}
		...
		{
			throw new \Exception('factoryFromXml invalid payment request type=' . \wp_kses($attr->nodeValue), (int)PaymentAbstract::ERROR_FACTORY_BY_XML_ORDER_TYPE_ATTR_NOT_FOUND);
		}
		...
		case PaymentAbstract::PAYMENT_TYPE_SMS:
			...
			break;
		default:
			throw new \Exception('factoryFromXml invalid payment request type=' . \wp_kses($attr->nodeValue), (int)PaymentAbstract::ERROR_FACTORY_BY_XML_INVALID_TYPE);
			break;
		}
	...
	protected function _parseFromXml(\DOMNode $elem)
	{
		...
		{
			throw new \Exception('Sms::_parseFromXml failed: empty order id', (int)self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new \Exception('Sms::loadFromXml failed: signature is missing', (int)self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
		}
	...
	public function encrypt($x509FilePath)
	{
		...
		if($publicKey === false)
		{
			...
			throw new \Exception(\wp_kses($errorMessage), (int)self::ERROR_LOAD_X509_CERTIFICATE);
		}
		...
		else
		{
			if(OPENSSL_VERSION_NUMBER >= 0x30000000)
			{
				...
				throw new \Exception(\wp_kses($errorMessage), (int)self::ERROR_REQUIRED_CIPHER_NOT_AVAILABLE);
			}
		}
		...
		if($result === false)
		{
			...
			throw new \Exception(\wp_kses($errorMessage), (int)self::ERROR_ENCRYPT_DATA);
		}

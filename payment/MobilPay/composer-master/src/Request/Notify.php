...
class Notify {
    ...
    public function loadFromXml(\DOMElement $elem) {
        ...
        if ($attr == null) {
            throw new \Exception('Notify::loadFromXml failed; mandatory crc attribute missing', (int)self::ERROR_LOAD_FROM_XML_CRC_ATTR_MISSING);
        }
        $this->_crc = $attr->nodeValue;
        $elems = $elem->getElementsByTagName('action');
        if ($elems->length != 1) {
            throw new \Exception('Notify::loadFromXml failed; mandatory action attribute missing', (int)self::ERROR_LOAD_FROM_XML_ACTION_ELEM_MISSING);
        }

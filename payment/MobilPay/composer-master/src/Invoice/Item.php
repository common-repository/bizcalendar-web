...
class Item
{
	...
	protected function loadFromXml(\DOMNode $elem)
	{
		...
		if($elems->length != 1)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid code element.', (int)self::ERROR_LOAD_FROM_XML_CODE_ELEM_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid name element.', (int)self::ERROR_LOAD_FROM_XML_NAME_ELEM_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid quantity element.', (int)self::ERROR_LOAD_FROM_XML_QUANTITY_ELEM_MISSING);
		}
		$this->quantity = doubleval(urldecode($elems->item(0)->nodeValue));
		if($this->quantity == 0)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid quantity value=' . (double)$this->quantity, (int)self::ERROR_LOAD_FROM_XML_QUANTITY_ELEM_EMPTY);
		}

		$elems = $elem->getElementsByTagName('price');
		if($elems->length != 1)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid price element.', (int)self::ERROR_LOAD_FROM_XML_PRICE_ELEM_MISSING);
		}
		$this->price = doubleval(urldecode($elems->item(0)->nodeValue));
		if($this->price == 0)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid price value=' . (double)$this->price, (int)self::ERROR_LOAD_FROM_XML_PRICE_ELEM_EMPTY);
		}

		$elems = $elem->getElementsByTagName('vat');
		if($elems->length != 1)
		{
			throw new \Exception('Item::loadFromXml failed! Invalid vat element.', (int)self::ERROR_LOAD_FROM_XML_VAT_ELEM_MISSING);
		}
	...
	public function createXmlElement(\DOMDocument $xmlDoc)
	{
		...
		{
			throw new \Exception('', (int)self::ERROR_INVALID_PARAMETER);
		}
		...
		{
			throw new \Exception('Invalid property', (int)self::ERROR_INVALID_PROPERTY);
		}
